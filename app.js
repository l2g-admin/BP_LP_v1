...
// --------------------------------------------------------------------------
// Guided nav rail (+ the sticky CTA, which rides the same active-slide
// signal). One dot per .fold as before, but everything is explicit
// now: big prev/next arrow buttons around the dots, a "1 / 6" counter,
// tappable dots, and the whole rail visible from slide 1 — this
// variant's hypothesis is that the deck was fine and only its
// navigation was undiscoverable. The same 0.6 settle ratio marks the
// active slide; snap guarantees a clean landing for every jump. The
// sticky bar logic is unchanged from production (on from #the-capsule,
// dormant escalation hook for a future id="the-proof" slide).
// --------------------------------------------------------------------------
const navrail = document.querySelector('.navrail');
const stickyCta = document.querySelector('.sticky-cta');
const folds = Array.from(document.querySelectorAll('.fold'));
if (navrail && folds.length > 1) {
  const goTo = (i) => {
    if (i >= 0 && i < folds.length) {
      folds[i].scrollIntoView({ behavior: 'smooth' });
    }
  };

  const upBtn = navrail.querySelector('.navrail__btn--up');
  const downBtn = navrail.querySelector('.navrail__btn--down');
  const countLabel = navrail.querySelector('.navrail__count');
  let active = 0;
  upBtn.addEventListener('click', () => goTo(active - 1));
  downBtn.addEventListener('click', () => goTo(active + 1));
  ...
  const railObserver = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      if (entry.intersectionRatio >= 0.6) {
        active = folds.indexOf(entry.target);
        countLabel.textContent = `${active + 1} / ${folds.length}`;
        upBtn.classList.toggle('navrail__btn--hidden', active === 0);
        downBtn.classList.toggle('navrail__btn--hidden', active === folds.length - 1);
        // The whole widget retires into the edge on the last slide.
        navrail.classList.toggle('navrail--collapsed', active === folds.length - 1);
        // Ink chrome vanishes on the dark slides; flip the rail to cream.
        navrail.classList.toggle(
          'navrail--dark',
...
// The slide-1 cue is a real button in this variant: tap advances to
// slide 2, same glide the arrows use.
document.querySelector('.scrollcue')?.addEventListener('click', () => {
  document.getElementById('the-molecule')?.scrollIntoView({ behavior: 'smooth' });
});
...
const KEEP_CUE_DELAY = {
  'the-molecule': 1500, // the compressed single arrival has landed
  'the-vessel': 2200,   // release has played (copy landed long before)
  'the-supply': 4600,   // the chart has finished drawing to age 70
  'the-source': 1800,   // copy landed with the scene; conversion loops on
};
